...
public class SensoryEvilPatches {
    ...
    @SpirePatch2(clz = SensoryStone_Evil.class, method = SpirePatch.CONSTRUCTOR, requiredModId = "downfall")
    public static class EventInit {
        @SpirePostfixPatch
        public static void addOption(SensoryStone_Evil __instance) {
            if (CardAugmentsMod.eventAddons) {
                ...
                if (enumElements == null) {
                    try {
                        Class<?> enumElement = Class.forName(SensoryStone_Evil.class.getName()+"$CurScreen");
    ...
    @SpirePatch2(clz = SensoryStone_Evil.class, method = "buttonEffect", requiredModId = "downfall")
    public static class AddButton {
        @SpireInsertPatch(locator = Locator.class)
        public static void add(SensoryStone_Evil __instance) {
            if (CardAugmentsMod.eventAddons) {
        ...
        public static class Locator extends SpireInsertLocator {
            ...
            public int[] Locate(CtBehavior ctBehavior) throws Exception {
                Matcher m = new Matcher.FieldAccessMatcher(SensoryStone_Evil.class, "screen");
    ...
    public static class ButtonLogic {
        @SpirePrefixPatch
        public static SpireReturn<?> buttonPress(SensoryStone_Evil __instance, int buttonPressed, Object ___screen) {
            if (CardAugmentsMod.eventAddons) {
                if (___screen == enumElements[1]) {
                    if (buttonPressed == myIndex) {
                        ...
                        ReflectionHacks.setPrivate(__instance, SensoryStone_Evil.class, "screen", enumElements[3]);
    ...
    public static void updateEvent(SensoryStone_Evil __instance) {
        if (CardAugmentsMod.eventAddons) {
            if (pickedCard && !AbstractDungeon.isScreenUp && !AbstractDungeon.gridSelectScreen.selectedCards.isEmpty()) {
                ...
                pickedCard = false;
                AbstractEvent.logMetric("downfall:SensoryStone", "Reject", null, null, null, Collections.singletonList(c.cardID), null, null, null, DAMAGE, 0, 0, 0, 0, 0);
            }
        }
    }
    ...
    public static class AddUpdateOverride {
        ...
        public static void addMethod(CtBehavior ctMethodToPatch) throws CannotCompileException, NotFoundException {
            ...
            updateMethod.insertAfter(SensoryEvilPatches.class.getName()+".updateEvent($0);");
        }
    }
}
