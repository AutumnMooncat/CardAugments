...
public class ErangMod extends AbstractAugment {
    ...
    public boolean validCard(AbstractCard card) {
        return cardCheck(card, c -> c.baseDamage > 0 && doesntUpgradeCost() && targetsEnemy(c) && c.type == AbstractCard.CardType.ATTACK && customCheck(c, check -> check.rawDescription.chars().filter(ch -> ch == '.' || ch == '。').count() == 1));
    }
